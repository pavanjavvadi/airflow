...
ss_took_date = datetime.now()
def take_screenshot(url: str, ti):
    driver = webdriver.Chrome('/opt/airflow/data/chrome_driver/chromedriver')
    driver.get(url)
    print(driver.title)
    ...
    button.send_keys(Keys.RETURN)
    time.sleep(1)
    ss = driver.save_screenshot(f'/opt/airflow/data/screenshots/ss-{ss_took_date}.png')
    driver.close()
...
with DAG(
    ...
    save_images = PythonOperator(
        ...
        op_kwargs={'ss': f'/opt/airflow/data/screenshots/ss-{ss_took_date}.png', 'bucket_name': 'ubuntu-airflow', 'file_name': 'screenshot.png'}
    )
